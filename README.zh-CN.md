...
- **图片模式**先在本地完成尺寸与像素预检；在网页端，用户明确同意后，图片会由服务端 Gemini 提炼为受约束、明确标注为单图推断的视觉系统。
...
| 安全控制 | URL 校验、重定向/请求复核、图片明确同意、魔数校验、输入上限、仅服务端 Gemini 密钥 |
...
2. 上传一张截图，先完成本地预检，再明确同意由 Gemini 生成带“单图推断”边界的风格规则；
...
- 先在本地预检上传图片；只有用户明确同意后才发送给 Gemini，服务端会校验类型、魔数、体积与像素上限，且绝不返回密钥。

如果要把 URL 采集能力暴露给不受信任的用户，请先阅读 [`SECURITY.md`](./SECURITY.md)。

> **部署边界：**生产容器、Gemini 服务端路径和 Cloud Run 清单已就绪，但 `uitoprompt.com` 尚未完成 DNS 切换。公开在线版会先开放图片分析；任意网页 URL 抓取会持续关闭，直到独立 Worker、队列、出网控制与公网限流都已部署并验证。
...
浏览器会先在本地进行预检。只有勾选明确同意后，图片才会发送到服务端供本次 Gemini 分析，且不会写入项目库；Python Skill 脚本只读取你明确提供的路径。不要提交包含秘密或个人数据的截图。
</details>
